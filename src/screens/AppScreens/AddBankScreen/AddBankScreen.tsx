...
export default function AddBackScreen(): JSX.Element {
  ...
  return (
    <View style={Styles.container} >
      ...
      <ScreenHeader screenName="Add Bank Account" onPress={onPressGoBack} />
      <View style={Styles.wrapper} >
        <Text style={Styles.headerText} >Add Bank Account</Text>
        <Text style={Styles.subHeaderText} >{'Account Should be registered with mobile\nNo. 0975462144'}</Text>
        <View style={Styles.searchbarContainer} >
          <SearchContainer containerStyle={{ paddingVertical: 7, borderRadius: 20, borderWidth: 1, borderColor: 'rgba(46, 46, 46, 0.3)', backgroundColor: AppStyles.colorGreyLight3 }} iconWidth={20} iconHeight={20} placeholder="Search other banks..." value="" onChangeValue={() => {}} iconContainerStyle={{ width: '12%' }} inputContainerStyle={{ width: '85%' }} inputStyle={{ fontFamily: AppStyles.fontPoppinsMedium, fontSize: 14 }} />
        </View>
        <View style={Styles.bankOptionContainer} >
          {banks.map((item, i) => (
            <TouchableOpacity activeOpacity={0.85} key={i.toString()} style={Styles.optionContainer} >
